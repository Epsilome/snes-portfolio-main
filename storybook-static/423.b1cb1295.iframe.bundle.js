    ...
    "./node_modules/tailwind-merge/dist/bundle-mjs.mjs": (
      ...
      const createClassGroupUtils = (config) => {
        ...
        getGroupIdForArbitraryProperty = (className) => {
          if (arbitraryPropertyRegex.test(className)) {
            const arbitraryPropertyClassName = arbitraryPropertyRegex.exec(className)[1],
              property = arbitraryPropertyClassName?.substring(0, arbitraryPropertyClassName.indexOf(":"))
            ...
            if ("string" != typeof classDefinition) {
              if ("function" == typeof classDefinition)
                return isThemeGetter(classDefinition)
                  ? void processClassesRecursively(classDefinition(theme), classPartObject, classGroupId, theme)
                  : void classPartObject.validators.push({ validator: classDefinition, classGroupId })
              Object.entries(classDefinition).forEach(([key, classGroup]) => {
                processClassesRecursively(classGroup, getPart(classPartObject, key), classGroupId, theme)
              })
            } else {
              ;("" === classDefinition ? classPartObject : getPart(classPartObject, classDefinition)).classGroupId =
                classGroupId
            }
          })
        },
            ...
            ? classGroupEntries.map(([classGroupId, classGroup]) => [
                ...
                classGroup.map((classDefinition) =>
                  "string" == typeof classDefinition
                    ...
                    ? Object.fromEntries(Object.entries(classDefinition).map(([key, value]) => [prefix + key, value]))
                    : classDefinition
                ),
              ])
            : classGroupEntries,
        createLruCache = (maxCacheSize) => {
          ...
          const update = (key, value) => {
            cache.set(key, value),
              cacheSize++,
              cacheSize > maxCacheSize && ((cacheSize = 0), (previousCache = cache), (cache = new Map()))
          }
          return {
            get(key) {
        ...
        createParseClassName = (config) => {
          const { separator, experimentalParseClassName } = config,
            ...
            parseClassName = (className) => {
              ...
              const baseClassNameWithImportantModifier =
                ...
                hasImportantModifier = baseClassNameWithImportantModifier.startsWith("!")
              return {
                modifiers,
          ...
          return experimentalParseClassName
            ? (className) => experimentalParseClassName({ className, parseClassName })
            : parseClassName
        },
        sortModifiers = (modifiers) => {
          ...
          return (
            modifiers.forEach((modifier) => {
              "[" === modifier[0]
                ? (sortedModifiers.push(...unsortedModifiers.sort(), modifier), (unsortedModifiers = []))
          ...
          const result = ((classList, configUtils) => {
            ...
            for (let index = classNames.length - 1; index >= 0; index -= 1) {
              const originalClassName = classNames[index],
                { modifiers, hasImportantModifier, baseClassName, maybePostfixModifierPosition } =
      ...
      const fromTheme = (key) => {
        ...
        getIsArbitraryValue = (value, label, testValue) => {
          ...
          return (
            !!result &&
            (result[1] ? ("string" == typeof label ? result[1] === label : label.has(result[1])) : testValue(result[2]))
          )
        },
        ...
        getDefaultConfig =
          ...
          () => {
            const colors = fromTheme("colors"),
              ...
              gradientColorStopPositions = fromTheme("gradientColorStopPositions"),
              ...
              getSpacingWithAutoAndArbitrary = () => ["auto", isArbitraryValue, spacing],
            ...
            return {
              ...
              classGroups: {
                ...
                "bg-image": [
                  { bg: ["none", { "gradient-to": ["t", "tr", "r", "br", "b", "bl", "l", "tl"] }, isArbitraryImage] },
                ],
                "bg-color": [{ bg: [colors] }],
